 ...
 *  files.
 *  GJC主要入口
 *  它提供构造新编译器和在一组源文件上运行新编译器的操作
 *
 *  <p><b>This is NOT part of any supported API.
 *  If you write code that depends on this, you do so at your own risk.
 *  This code and its internal interfaces are subject to change or
 *  deletion without notice.
 *  如果你编写依赖于此的代码，那么你就会在你自己的风险中这样做，这段代码及其内部接口可能会在没有通知的情况下更改或删除
 *  </b>
 *
 */
public class JavaCompiler implements ClassReader.SourceCompleter {
    ...
    protected static enum CompilePolicy {
        ...
        /**
         * Groups the classes for each source file together,
         * 将每个源文件的类分组在一起
         * then process each group in a manner equivalent to the {@code SIMPLE} policy.
         * 然后以等价于{@code SIMPLE}策略的方式处理每个组。
         * This means no output will be generated if there are any errors in any of the classes in a source file.
         * 这意味着如果源文件中的任何类中有任何错误，就不会生成输出
         */
        BY_FILE,
         ...
         * Means output might be generated for some classes in a compilation unit and not others.
         * 意味着可以为编译单元中的某些类生成输出，而不是其他类
         */
        BY_TODO;
    ...
    public JavaCompiler(Context context) {
        ...
        sourceOutput  = options.isSet(PRINTSOURCE); //// used to be -s 命令行选项javac -printsource
        ...
        encoding      = options.get(ENCODING);//-encoding <编码> 指定源文件使用的字符编码
        lineDebugInfo = options.isUnset(G_CUSTOM) ||// 是否生成行号信息属性(debug)
                        options.isSet(G_CUSTOM, "lines");
        genEndPos     = options.isSet(XJCOV) ||// 命令行选项:javac -Xjcov
        ...
        if (attrParseOnly)
            ...
            // 正常情况下都是只能是默认的编译策略DEFAULT_COMPILE_POLICY = CompilePolicy.BY_TODO
        ...
        completionFailureName =
            ...
            //什么时候停止编译的策略，null就是一直处理到第一个错误出现，标准options选项里没有这个策略
            shouldStopPolicy =
    ...
    protected JCCompilationUnit parse(JavaFileObject filename, CharSequence content) {
        long msec = now();
        // 用TreeMaker(make)生成一颗JCCompilationUnit树
        ...
        if (content != null) {
            ...
            // com.sun.tools.javac.api.JavacTaskImpl#setTaskListener(TaskListener taskListener)
            if (taskListener != null) {
                TaskEvent e = new TaskEvent(TaskEvent.Kind.PARSE, filename);
                taskListener.started(e);
            }
            // 建立语法分析实例(newParser()函数内部先建立了词法分析实例)
            Parser parser = parserFactory.newParser(content, keepComments(), genEndPos, lineDebugInfo);
            /*
             * 这里是个多态调用，Parser是parser实际类型的超类
             * 默认情况下parser的类型为com.sun.tools.javac.parser.JavacParser
             * 当要有注解处理器或javac -Xjcov的时候parser类型为com.sun.tools.javac.parser.EndPosParser
             * 详细请参考parserFactory.newParse内注释
             */
    ...
    public JCTree.JCCompilationUnit parse(JavaFileObject filename) {
        ...
        try {
            // readSource(filename):返回filename的一个char值的可读序列(CharSequence)
...
//      System.err.println("completing " + c);//DEBUG
     ...
     *  编译源文件列表中的源代码
     * @param sourceFileObjects file objects to be compiled 等待被编译的源代码文件类表
     * @param classnames class names to process for annotations 要运行注解处理程序的名称
     * @param processors user provided annotation processors to bypass 用户提供的注解处理器绕过默认的搜索进程
                        ...
                        Iterable<? extends Processor> processors)
    {
        ...
        // 一个JavaCompiler只能被使用一次,如果被使用过则会抛出一个异常
        if (hasBeenUsed)
        ...
        //强制设置-Xlint:-options，这样从此时开始就不会生成关于 命令行选项 的警告
   ...
   public List<JCCompilationUnit> parseFiles(Iterable<JavaFileObject> fileObjects) {
       ...
       if (shouldStop(CompileState.PARSE))
        ...
        //parse all files
        // JCCompilationUnit是一个类的最顶层语法树，trees里面维持着所有编译好的类文件的最顶层语法树
        ListBuffer<JCCompilationUnit> trees = lb();
       // HashSet filesSoFar用来剔除fileObjects中可能存在的重复的项目
        Set<JavaFileObject> filesSoFar = new HashSet<JavaFileObject>();
        for (JavaFileObject fileObject : fileObjects) {
            if (!filesSoFar.contains(fileObject)) {
                filesSoFar.add(fileObject);
                //parse开始:循环parse每一个类、接口或枚举，
                // 返回的每个类、接口或枚举对应的最顶层抽象语法树JCCompilationUnit
    ...
    public void initProcessAnnotations(Iterable<? extends Processor> processors) {
        ...
        //-proc:{none, only}指定是否执行注解处理   (和/或)   编译
        if (options.isSet(PROC, "none")) {
            // 禁止执行注解处理
            processAnnotations = false;
        } else if (procEnvImpl == null) {
            // procEnvImpl持有并管理需要支持 注释处理器
    ...
    /**
     * 如果命令行明确的输入了跟注解处理器相关的参数的话，返回true，否则返回false
     * @param options
     * @return
     */
